...
// follow the current album art; in 'custom' mode they come from settings.
...
export const ThemeManager = GObject.registerClass({
    ...
    _init(settings) {
        ...
        this._settingsIds = ['accent-mode', 'accent-start', 'accent-end'].map((key) =>
            settings.connect(`changed::${key}`, () => this.emit('changed'))
        );
    }

    get accentA() {
        if (this._adaptive && this._artColors) return this._artColors[0];
        return this._settings.get_string('accent-start');
    }

    get accentB() {
        if (this._adaptive && this._artColors) return this._artColors[1];
