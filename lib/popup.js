...
class MediaPopup extends GObject.Object {
    ...
    _init(panelButton, watcher, theme) {
        super._init();
    ...
    _applyArtStyle() {
        const base = 'border-radius: 14px; margin-bottom: 14px;';
    ...
    _toggleBtnStyle(active, size) {
        const dims = `width: ${size}px; height: ${size}px; font-size: ${Math.floor(size * 0.35)}px;`;
        return active
