...
export const MprisWatcher = GObject.registerClass({
    ...
    async _addPlayer(busName) {
        ...
        try {
            ...
            const entry = { proxy, handlerId: 0, lastChanged: Date.now() };
            this._players.set(busName, entry);

            entry.handlerId = proxy.connect('g-properties-changed', () => {
                if (!this._players.has(busName)) return; // stale: player was removed
